    ...
    def dump(self,
        ...
        for element in self._locations_list:
            ...
            with open(file_path, "w", encoding="utf-8") as file:
                ...
                initial_dump += knx_build_string(f"#& File generated by HomeAssistantKNXAutomaticImport v{__version__}")
                initial_dump += knx_build_string(f"#& Date: {datetime.now()}")
                initial_dump += knx_build_string(f"#&")
                initial_dump += knx_build_string(f"#& Project name {HAKAIConfiguration.get_instance().project.info.name}")
                initial_dump += knx_build_string(f"#& Project date {HAKAIConfiguration.get_instance().project.info.last_modified}")
                initial_dump += knx_build_string(f"#& Project tool {HAKAIConfiguration.get_instance().project.info.tool_version}")
                initial_dump += knx_build_string(f"#& xknxproject version {HAKAIConfiguration.get_instance().project.info.xknxproject_version}")
                initial_dump += knx_build_string(f"#&")
                initial_dump += knx_build_string(f"#& Location {element.name}")
                initial_dump += knx_build_string(f"#&")
                initial_dump += knx_build_string(f"#& HomeAssistantKNXAutomaticImport options")
                initial_dump += knx_build_string(f"#&    . location separator: '{HAKAIConfiguration.get_instance().location_separator}'")
                initial_dump += knx_build_string(f"#&    . suppress project name: {HAKAIConfiguration.get_instance().suppress_project_name}")
                initial_dump += knx_build_string(f"#&    . replace spaces: '{HAKAIConfiguration.get_instance().replace_spaces}'")
                initial_dump += knx_build_string(f"#&    . remove keyword: {HAKAIConfiguration.get_instance().remove_keyword}")
                initial_dump += knx_build_string(f"#&    . name pattern: '{HAKAIConfiguration.get_instance().name_pattern}'")
                initial_dump += knx_build_string(f"#&")
                initial_dump += knx_build_string("#&","#")
                initial_dump += "\n\n"
